...
export const zUpdateContestInput = z
  ...
  .check((ctx) => {
    ...
    if (ctx.value.startAt < oneHourFromNow) {
      ctx.issues.push({
        code: "custom",
    ...
    if (ctx.value.contestType != "live" && !ctx.value.endAt) {
      ctx.issues.push({
        code: "invalid_value",
    ...
    if (
      ctx.value.contestType != "live" &&
      ctx.value.endAt.getTime() - ctx.value.startAt.getTime() < minDurationMs
    ) {
      ctx.issues.push({
        code: "custom",
